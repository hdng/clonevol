...
#'
#' @description Return a list of p-value and confidence intervals of X-(X1+X2)
...
subclonal.test <- function(vaf.col.name, parent.cluster, sub.clusters=NULL,
  ...
  if (is.null(boot) || length(boot) == 0){
    ...
    if (is.null(sub.clusters)){
      ...
      mean.free.vaf = cdf$vaf[cdf$lab==parent.cluster] - sum(cdf$vaf[cdf$lab %in% sub.clusters])
    ...
    confident.level.non.negative = ifelse(lower.free.vaf >= 0, confident.level,
                                      sum(free.vaf >= 0 & free.vaf <= upper.free.vaf)/length(free.vaf)
                                      )
    free.vaf.ci.str = ifelse(lower.free.vaf > 0,
                           paste0(lower.free.vaf.fmt, ' - ', upper.free.vaf.fmt),
...
testttt <- function(){
  ...
  dir.create(out.dir)
  for (s in vaf.col.names){
    draw.sample.clones.all(x$models[[s]], paste0(out.dir, '/', s, '.tree'), object.to.plot='tree')
  }
