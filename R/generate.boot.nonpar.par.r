...
#' @param model: specifies the statistical model used in bootstrap resampling (can be normal, beta, beta-binomial, or non-parametric)

#Last update: Steven Mason Foltz 2015-03-17
...
generate.boot.nonpar.par <- function(variants, cluster.col.name='cluster', vaf.col.names=NULL, vaf.in.percent=TRUE, num.boots=1000, zero.sample=NULL, model=NULL){
  ...
  for (vaf.col.name in vaf.col.names){
    ...
    for (cl in clusters){
      ...
      #uses general normal i.e. could produce values below 0 or above 1 (bad)
      if(model == "normal"){
      ...
      if(model == "normal-truncated"){
        library(truncnorm) #use truncnorm library
        for (b2 in 1:num.boots){ #b2 loop parameter since b in rtruncnorm()
          #use mean and standard deviation as normal MLEs
          s.mean <- mean(rtruncnorm(n=boot.size, a=0, b=1, mean=this.mean, sd=this.sd))
      ...
      if(model == "beta-binomial"){
        ...
        for(b in 1:num.boots){
          ...
          s.mean <- mean(rbinom(n=boot.size, size=100, prob=beta.probs))/100
          sample.boot.means[b,cl] <- s.mean
        }
        next
      }
