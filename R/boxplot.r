...
multiplot <- function(..., plotlist=NULL, file, cols=1, layout=NULL, horizontal=F, e=0.15) {
  require(grid)
  ...
  for (ii in 1:length(vaf.col.names)){
    ...
    if (box && violin){
      p = (p + geom_violin(scale='width', color=boxColor,
                           linetype=violin.line.type, size=violin.line.size)
    ...
    if (!is.null(highlight)){
      ...
      if (!is.null(highlight.color.col.name)){
        p = p + geom_point(data=df.hi, aes_string(x = 'newX', y=yName,
                                                  size=sizeName, color=highlight.color.col.name),
                           shape=1, show_guide=T)
      }else{
        p = p + geom_point(data=df.hi, aes_string(x = 'newX', y=yName,
                                                  size=sizeName), color=highlight.color, shape=1,
                           show_guide=T)

      }
      if (!is.null(sizeName)){
        size.breaks = c(0, 50, 100, 200, 300, 500)
        size.breaks = size.breaks[size.breaks <= max.highlight.size.value]
        size.labels = size.breaks
        size.labels[length(size.labels)] = paste0('>',
      ...
      p = p + geom_text(data=df.hi, aes_string(x=cluster.col.name, y=yName,
                                               label=highlight.note.col.name, hjust='hjust'),
...
boxplot.example <- function(){
  v = read.table('variants.tsv', header=T, sep='\t', quote='', stringsAsFactors=F)
